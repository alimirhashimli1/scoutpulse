...
keys:
	...
	@openssl genpkey -algorithm RSA -pkeyopt rsa_keygen_bits:2048 -out .jwt-private.pem 2>/dev/null
	@openssl rsa -in .jwt-private.pem -pubout -out .jwt-public.pem 2>/dev/null
	@{ \
		echo ""; \
		echo "# Generated by 'make keys'. Never commit these."; \
		printf 'JWT_PRIVATE_KEY="'; awk '{printf "%s\\n", $$0}' .jwt-private.pem; echo '"'; \
		printf 'JWT_PUBLIC_KEY="'; awk '{printf "%s\\n", $$0}' .jwt-public.pem; echo '"'; \
	} >> .env
