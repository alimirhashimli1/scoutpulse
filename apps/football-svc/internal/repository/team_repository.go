...
func (r *postgresTeamRepository) GetByID(ctx context.Context, id string) (*domain.Team, error) {
	...
	query := `SELECT id, league_id, name, fan_badge, created_at FROM teams WHERE id = $1`
...
func (r *postgresTeamRepository) ListByLeague(ctx context.Context, leagueID string) ([]domain.Team, error) {
	...
	query := `SELECT id, league_id, name, fan_badge, created_at FROM teams WHERE league_id = $1 ORDER BY name`
...
func (r *postgresTeamRepository) Create(ctx context.Context, team *domain.Team) error {
	query := `INSERT INTO teams (league_id, name, fan_badge) VALUES ($1, $2, $3) RETURNING id, created_at`
	err := r.db.QueryRowContext(ctx, query, team.LeagueID, team.Name, team.FanBadge).Scan(&team.ID, &team.CreatedAt)
	return err
}

func (r *postgresTeamRepository) Update(ctx context.Context, team *domain.Team) error {
	query := `UPDATE teams SET league_id = $1, name = $2, fan_badge = $3 WHERE id = $4`
	_, err := r.db.ExecContext(ctx, query, team.LeagueID, team.Name, team.FanBadge, team.ID)
	return err
}
